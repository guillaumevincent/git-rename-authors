...
print('warning use it only if you know what you are doing, no warranty')

process = subprocess.run("git log --pretty=\"%an;%ae%n%cn;%ce\" | sort | uniq", stdout=subprocess.PIPE, shell=True)

stdout = process.stdout.decode('utf-8')
if len(stdout) == 0:
    tkinter.messagebox.showerror(title='Error', message='Not a git repository (or any of the parent directories): .git')
    sys.exit(-1)
